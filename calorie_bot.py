...
# ===================== НАСТРОЙКИ =====================
BOT_TOKEN = "ТОЙ_СЮДА_ТОКЕН_ОТ_BOTFATHER"
GEMINI_API_KEY = "ТОЙ_СЮДА_КЛЮЧ_ИЗ_GOOGLE_AI_STUDIO"


genai.configure(api_key=GEMINI_API_KEY)
model = genai.GenerativeModel('gemini-2.5-flash')   # актуально на 2026
...
cursor = conn.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS goals (user_id INTEGER PRIMARY KEY, calories INTEGER, protein INTEGER, fat INTEGER, carbs INTEGER)''')
cursor.execute('''CREATE TABLE IF NOT EXISTS meals (id INTEGER PRIMARY KEY, user_id INTEGER, date TEXT, meal_text TEXT, calories REAL, protein REAL, fat REAL, carbs REAL)''')
conn.commit()
...
Анализируй фото или описание. Если вес не указан — оцени по ладони взрослого мужчины (\~150 г).
Учитывай контекст: "Командировка" — добавь +10-15% жиров.
Верни ТОЛЬКО JSON без всякого текста:
{
...
Предложи ровно 2–3 варианта. 
Если >19:00 и белок <160 — только чистый белок.
Если тренировка — углеводный перекус за 1.5 ч до зала.
...
async def start(message: Message):
    await message.answer(
        ...
        "Кидай фото еды или текстовое описание.\n"
...
@dp.message(Command("endday"))
async def endday(message: Message):
    eaten_p = get_today_stats(message.from_user.id)[1]
    await message.answer("✅ День закрыт.\n" + ("Успешно! Белок в норме." if eaten_p >= 160 else "Завтра нажми на белки."))
...
async def handle_food(message: Message):
    ...
    training = any(word in text for word in ["тренировка", "тренировк", "зал", "gym"])
    trip = any(word in text for word in ["командировка", "командировк"])
    ...
    try:
        ...
        cursor.execute(
            ...
            (user_id, date.today().isoformat(), message.text or "Фото", total["calories"], total["protein"], total["fat"], total["carbs"])
        )
        conn.commit()
...
"""
        ...
        rec_prompt = RECOMMEND_PROMPT.format(
            ...
            training="да" if training else "нет",
        ...
        await message.answer("❌ Не удалось распознать. Опиши подробнее (вес, состав) или пришли другое фото.")
